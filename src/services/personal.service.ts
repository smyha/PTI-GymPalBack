...
import { sendSuccess, sendError, sendNotFound, sendCreated } from '../shared/utils/response.js';
 ...
 */
export async function getPersonalInfo(c: Context) {
  try {
 ...
 */
export async function upsertPersonalInfo(c: Context, body: any) {
  try {
    const userId = c.get('userId');
 ...
 */
export async function getFitnessProfile(c: Context) {
  try {
    const userId = c.get('userId');
    ...
    if (error && error.code !== 'PGRST116') {
      ...
      return sendError(
        c,
        ERROR_CODES.DATABASE_ERROR,
        'Failed to fetch fitness profile',
        500,
        error.message
      );
    }
 ...
 */
export async function upsertFitnessProfile(c: Context, body: any) {
  try {
    const userId = c.get('userId');
    ...
    if (existing) {
      ...
      if (error) {
        ...
        return sendError(
          c,
          ERROR_CODES.DATABASE_ERROR,
          'Failed to create fitness profile',
          500,
          error.message
        );
      }
 ...
 */
export async function getCompletePersonalData(c: Context) {
  try {
    const userId = c.get('userId');
