...
routinesHandler.get(
  '/',
  authMiddleware,
  validationMiddleware({ query: RoutineSchemas.listRoutinesQuery }),
  async (c) => {
    try {
 ...
 *         description: Internal error
 */
routinesHandler.post(
  '/',
  authMiddleware,
  validationMiddleware({ body: RoutineSchemas.createRoutineBody }),
  async (c) => {
    try {
...
routinesHandler.get(
  '/:id',
  authMiddleware,
  validationMiddleware({ params: RoutineSchemas.getRoutineParams }),
  async (c) => {
    try {
...
routinesHandler.get(
  '/search',
  authMiddleware,
  validationMiddleware({ query: RoutineSchemas.searchRoutinesQuery }),
  async (c) => {
    try {
      const query = c.get('validatedQuery');
...
routinesHandler.delete(
  '/:id/exercises/:exerciseId',
  authMiddleware,
  validationMiddleware({ params: RoutineSchemas.updateRoutineExerciseParams }),
  async (c) => {
    try {
      const params = c.get('validatedParams');
...
routinesHandler.get(
  '/trending',
  authMiddleware,
  validationMiddleware({ query: RoutineSchemas.trendingRoutinesQuery }),
  async (c) => {
    try {
      const query = c.get('validatedQuery');
