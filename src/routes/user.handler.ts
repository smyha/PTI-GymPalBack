...
userHandler.put(
  '/profile',
  authMiddleware,
  validationMiddleware({ body: UserSchemas.updateProfileBody }),
  async (c) => {
    try {
...
userHandler.get(
  '/search',
  validationMiddleware({ query: UserSchemas.searchUsersQuery }),
  async (c) => {
    try {
      const query = c.get('validatedQuery') as { page?: string; limit?: string; q: string };
...
userHandler.get(
  '/:id',
  validationMiddleware({ params: UserSchemas.getUserParams }),
  async (c) => {
    try {
...
userHandler.get(
  '/:id/achievements',
  validationMiddleware({ params: UserSchemas.getUserParams }),
  validationMiddleware({ query: UserSchemas.paginationQuery }),
  async (c) => {
    try {
      const params = c.get('validatedParams') as { id: string };
      const query = c.get('validatedQuery') as Record<string, unknown>;
...
userHandler.delete(
  '/account',
  authMiddleware,
  validationMiddleware({ body: UserSchemas.deleteAccountBody }),
  async (c) => {
    try {
      const body = c.get('validatedBody');
