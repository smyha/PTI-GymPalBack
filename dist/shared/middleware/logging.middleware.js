...
export const loggingMiddleware = async (c, next) => {
    ...
    try {
        ...
        // Log structured data
        logRequest({ method, url, headers: { 'user-agent': userAgent }, connection: { remoteAddress: ip } }, { statusCode }, duration);
...
export const authLoggingMiddleware = async (c, next) => {
    const start = Date.now();
    const method = c.req.method;
    const url = c.req.url;
    const ip = c.req.header('x-forwarded-for') || c.req.header('x-real-ip') || 'unknown';
...
export const apiLoggingMiddleware = async (c, next) => {
    const start = Date.now();
    const method = c.req.method;
    const url = c.req.url;
    const userAgent = c.req.header('user-agent') || 'unknown';
